...
if not defined current_version (echo. & echo  Download Zoom to "%dir%" ? & echo. & pause
) else (
...
xcopy "%temp%\zoom\Program Files (64-bit) Folder\Zoom\bin\*" "%dir%" /y /e /i /q >nul
...
reg add "HKCU\SOFTWARE\Classes\zoommtg\DefaultIcon" /ve /d "\"%dir%Zoom.exe\",1" /f >nul
reg add "HKCU\SOFTWARE\Classes\zoommtg\shell\open\command" /ve /d "\"%dir%Zoom.exe\" \"--url=%%1\"" /f >nul
