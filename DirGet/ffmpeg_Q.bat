...
if not defined current_version (echo. & echo  Download FFmpeg to "%dir%" ? & echo. & pause
) else (echo. & echo  Current version: v%current_version% & echo  Checking for updates...)

:: getting URL, filename and latest_ver
set "ps_cmd=$r=Invoke-RestMethod 'https://api.github.com/repos/GyanD/codexffmpeg/releases/latest'; $a=$r.assets|?{$_.name -like '*essentials_build.zip'}|select -f 1; echo $r.tag_name; echo $a.browser_download_url; echo $a.name"
...
if "%url%"=="" (echo  Error: Could not find download URL. & echo  Try manual: https://github.com/GyanD/codexffmpeg/releases & pause & exit /b)
