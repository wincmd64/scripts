...
if errorlevel 1 (echo. & echo  Unable to reach www.irfanview.info. Check your internet connection. & echo. & pause & goto check_site)
...
if exist "%app%" (
    ...
    for /f "tokens=2" %%v in ('curl.exe -s "https://www.irfanview.com/main_history.htm" ^| findstr /i "CURRENT.VERSION"') do (set "latest_version=%%v")
    cls
)
:: update logic
...
if exist "%temp%\%mainZip%" (tar -xf "%temp%\%mainZip%" 2>nul) else (echo. & echo  where %mainZip% ? & pause)
if exist "%temp%\%pluginsZip%" (tar -xf "%temp%\%pluginsZip%" -C "Plugins" 2>nul) else (echo. & echo  where %pluginsZip% ? & pause)
if exist "%temp%\irfanview_skin_iconshock.zip" (tar -xf "%temp%\irfanview_skin_iconshock.zip" -C "Toolbars" 2>nul) else (echo. & echo  where irfanview_skin_iconshock.zip ? & pause)
if exist "%temp%\irfanview_lang_ukrainian.zip" (tar -xf "%temp%\irfanview_lang_ukrainian.zip" -C "Languages" 2>nul) else (echo. & echo  where irfanview_lang_ukrainian.zip ? & pause)
if not exist "i_view64.ini" (
    ...
    :: to UTF-16
    powershell -command "Get-Content 'temp.txt' | Out-File 'i_view64.ini' -Encoding Unicode; Remove-Item 'temp.txt'"
)
