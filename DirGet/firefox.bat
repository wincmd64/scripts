...
for /f "tokens=*" %%a in ('powershell -C "$req = [System.Net.HttpWebRequest]::Create('%url%'); $req.AllowAutoRedirect = $false; $res = $req.GetResponse(); $res.GetResponseHeader('Location'); $res.Close()"') do set "final_url= %%a"
for /f "tokens=*" %%v in ('powershell -C "if ('%final_url%' -match 'releases/([^/]+)/') { $matches[1] }"') do set "latest_version=%%v"
cls
...
color A & echo. & echo. & echo  DOWNLOADED. Now launching Firefox... & echo.
