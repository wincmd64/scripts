...
reg add "HKCU\Software\Classes\*\shell\wincmd64_Hash\shell\01_SHA1\command" /ve /d "powershell.exe -NoP -C \"Write-Host ' Calculating checksum...'; $h = (Get-FileHash -LiteralPath '%%1' -Algorithm SHA1).Hash; $h ^| Set-Clipboard; Clear-Host; Write-Host $h `n`n SHA1 copied!`n; sleep 5\"" /f
...
reg add "HKCU\Software\Classes\*\shell\wincmd64_Hash\shell\02_SHA256\command" /ve /d "powershell.exe -NoP -C \"Write-Host ' Calculating checksum...'; $h = (Get-FileHash -LiteralPath '%%1' -Algorithm SHA256).Hash; $h ^| Set-Clipboard; Clear-Host; Write-Host $h `n`n SHA256 copied!`n; sleep 5\"" /f
...
reg add "HKCU\Software\Classes\*\shell\wincmd64_Hash\shell\03_SHA384\command" /ve /d "powershell.exe -NoP -C \"Write-Host ' Calculating checksum...'; $h = (Get-FileHash -LiteralPath '%%1' -Algorithm SHA384).Hash; $h ^| Set-Clipboard; Clear-Host; Write-Host $h `n`n SHA384 copied!`n; sleep 5\"" /f
...
reg add "HKCU\Software\Classes\*\shell\wincmd64_Hash\shell\04_SHA512\command" /ve /d "powershell.exe -NoP -C \"Write-Host ' Calculating checksum...'; $h = (Get-FileHash -LiteralPath '%%1' -Algorithm SHA512).Hash; $h ^| Set-Clipboard; Clear-Host; Write-Host $h `n`n SHA512 copied!`n; sleep 5\"" /f
...
reg add "HKCU\Software\Classes\*\shell\wincmd64_Hash\shell\05_MD5\command" /ve /d "powershell.exe -NoP -C \"Write-Host ' Calculating checksum...'; $h = (Get-FileHash -LiteralPath '%%1' -Algorithm MD5).Hash; $h ^| Set-Clipboard; Clear-Host; Write-Host $h `n`n MD5 copied!`n; sleep 5\"" /f
