...
if not defined current_version (echo. & echo  Download Notepad++ to "%~dp0" ? & echo. & pause
...
robocopy "%temp%\npp_update\localization" "%~dp0localization" english.xml russian.xml ukrainian.xml /move /r:0 /w:0 >nul
if exist "notepad++.exe" (
    :: ignore root *.xml
    robocopy "%temp%\npp_update" "%~dp0." /move /xf *.xml /xd localization updater /r:0 /w:0 >nul
) else (
