...
echo.
echo   [1] download "%file%" to "%DOWNLOADS%"
echo   [2] create portable installation in "%DOWNLOADS%\totalcmd"
echo   [3] update custom path (admin rights required)
echo   [4] update current instalation: "%commander_path%" (admin rights required)
echo.
...
if not exist "%temp%\%file%" (
    ...
    curl.exe "%url%" -RLO# --output-dir "%temp%"
...
:Option_1
call :download_temp
copy /y "%temp%\%file%" "%DOWNLOADS%\%file%"
...
:Option_2
call :download_temp
if not exist "%DOWNLOADS%\totalcmd" md "%DOWNLOADS%\totalcmd"
tar -xf "%temp%\%file%" -C "%DOWNLOADS%\totalcmd" 2>nul
curl.exe "https://raw.githubusercontent.com/wincmd64/blog/refs/heads/main/wincmd.ini" -#O --output-dir "%DOWNLOADS%\totalcmd"
...
:Option_3
call :download_temp
for /f "delims=" %%A in ('powershell -NoP "Add-Type -AssemblyName System.Windows.Forms; $dlg=New-Object System.Windows.Forms.FolderBrowserDialog; if($dlg.ShowDialog() -eq 'OK'){ $dlg.SelectedPath }"') do set "FOLDER_SOURCE=%%A"
if not defined FOLDER_SOURCE echo  Cancelled. & echo. & pause & exit
echo  Selected: "%FOLDER_SOURCE%"
