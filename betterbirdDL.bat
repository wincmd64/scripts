...
echo. & echo  Loading ...
:: get link
for /f "usebackq tokens=*" %%a in (`powershell -NoP -C "(Invoke-WebRequest 'https://www.betterbird.eu/downloads/getloc.php?os=win&lang=en-US&version=release&portable=true' -UseBasicParsing).Content.Trim()"`) do set "DL_URL=%%a"
:: get server filename
for %%i in ("%DL_URL%") do set "filename=%%~nxi"
:: get server ver
for /f "usebackq tokens=*" %%v in (`powershell -NoP -C "if ('%DL_URL%' -match '(\d+\.\d+\.\d+[-a-z0-9]*)') { $Matches[1] }"`) do set "VERSION=%%v"
:: get local ver
...
if not exist "%filename%" (
    ...
    powershell -C "Invoke-WebRequest -Uri '%DL_URL%' -OutFile '%filename%'"
