...
### context_menu_extras/
Скрипты в этой папке используют только встроенный ф-л ОС. При запуске будет добавлен соотв. пункт в контекстное меню. С ключом `/u` можно откатить изменения.

* **desktop_reminder.cmd**. Очень простая напоминалка - условно, не забыть выключить пелемени через 5 минут.
